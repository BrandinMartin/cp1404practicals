...
numbers = [int(number) for number in almost_numbers]  # 'number' seemed more appropriate than 'almost_number'
print(numbers)

large_numbers = [number for number in numbers if number > 9]
print(large_numbers)

long_full_names = [name.split()[1] for name in full_names if len(name) > 11]  # Selects last names from full names > 11
last_names_string = ', '.join(long_full_names)  # join method to create a string
